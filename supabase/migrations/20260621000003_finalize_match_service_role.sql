-- Allow the automated sync (service_role key) to call finalize_match.
-- The service_role key has no JWT user, so auth.uid() is NULL and is_admin_or_above()
-- returns false. Adding the auth.role() branch lets the sync proceed safely —
